...
Python script to extract repertoire files from a player's game database.  Adds play percentages that can be read by the platform Noctie.ai so if you upload the resulting repertoire to Noctie and play against it, it will make different moves at roughly the same frequency as the player.  Then when it exits the repertoire it plays on its own.
