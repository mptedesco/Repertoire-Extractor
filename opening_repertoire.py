...
with branching variations and statistical annotations.
    ...
    def build_variation_tree(self, game: chess.pgn.Game, openings: Dict[str, List[chess.pgn.Game]], 
        ...
        for opening_moves, games in openings.items():
            ...
            for i, san_move in enumerate(moves):
                ...
                if i == len(moves) - 1:
                    current_level[san_move]['games'].extend(games)
                
                current_level = current_level[san_move]['children']
        
        # Calculate total games reaching the root position
        total_root_games = sum(
            len(move_data.get('games', [])) + sum(
                self.count_games_in_subtree(child) for child in move_data['children'].values()
            )
            for move_data in move_tree.values()
        )
        
        # Convert tree to PGN variations
        self.add_variations_to_node(game, move_tree, chess.Board(), player_name, color, total_root_games)
    
    def add_variations_to_node(self, node: chess.pgn.GameNode, move_tree: dict, 
                             board: chess.Board, player_name: str, color: str,
                             total_parent_games: int) -> None:
        """Recursively add variations to a PGN node."""
        # Sort moves by frequency (number of games that reached this position)
        def get_total_games(move_data):
            total = len(move_data.get('games', []))
            for child in move_data['children'].values():
                total += self.count_games_in_subtree(child)
            return total
        
        sorted_moves = sorted(move_tree.items(), 
                            key=lambda x: get_total_games(x[1]), 
                            reverse=True)
        ...
        for san_move, move_data in sorted_moves:
            try:
                ...
                # Calculate %op: how often this move was played from the parent position
                move_games = get_total_games(move_data)
                if total_parent_games > 0:
                    op_fraction = move_games / total_parent_games
                else:
                    op_fraction = 0.0
                
                # Build comment starting with [%op X.X]
                comment_parts = []
                
                # Round to 1 decimal, but use 1.0 for 100%
                op_str = f"{op_fraction:.1f}" if op_fraction == 1.0 or op_fraction == 0.0 else f"{op_fraction:.1f}"
                comment_parts.append(f"[%op {op_str}]")
                
                # Add game statistics if there are games ending at this position
                if move_data['games']:
                ...
                if move_data['children']:
                    self.add_variations_to_node(variation_node, move_data['children'], 
                                              board_copy, player_name, color, move_games)
                
            except Exception as e:
                print(f"Warning: Could not parse move '{san_move}': {e}")
                continue
    
    def count_games_in_subtree(self, move_data: dict) -> int:
        """Count total games in a subtree."""
        total = len(move_data.get('games', []))
        for child in move_data['children'].values():
            total += self.count_games_in_subtree(child)
        return total
